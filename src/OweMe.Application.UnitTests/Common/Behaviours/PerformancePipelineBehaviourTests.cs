...
public class PerformancePipelineBehaviourTests
{
    ...
    const int delayThreshold = 500; // milliseconds
    const int totalDelay = delayThreshold + 1;
    const string expectedTimeUnit = "ms";
    ...
    public async Task Handle_Should_Log_Performance_When_Exceeding_Threshold()
    {
        ...
        Task<string> NextWithDelay(CancellationToken cancellationToken) => Task.Delay(totalDelay, cancellationToken).ContinueWith(_ => "response", cancellationToken);
        ...
        loggerMock.Verify(
            x => x.Log(
                ...
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((object v, Type t) => v.ToString().Contains("Handled TestRequest in") && AssertTime(v.ToString(), totalDelay, expectedAccuracyPercent, expectedTimeUnit)),
                null,
        ...
        loggerMock.Verify(
            x => x.Log(
                LogLevel.Warning,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString().Contains($", exceeding the threshold of {delayThreshold} ms")),
                null,
                It.IsAny<Func<It.IsAnyType, Exception, string>>()),
            Times.Once);
    }
    ...
    public async Task Handle_Should_Log_Performance_When_Exceeding_Threshold_WithException()
    {
        ...
        // Simulate a long-running operation
        Task<string> NextWithDelayAndException(CancellationToken cancellationToken) => Task.Delay(600, cancellationToken).ContinueWith<string>(_ => throw exception, cancellationToken);

        // Act and Assert
        await Should.ThrowAsync<Exception>(() => behaviour.Handle(request, NextWithDelayAndException, CancellationToken.None));
    ...
    public async Task Handle_ShouldNot_Log_Performance_When_Not_Exceeding_Threshold()
    {
        ...
        Task<string> NextWithShortDelay(CancellationToken cancellationToken) => Task.Delay(100, cancellationToken).ContinueWith(_ => "response", cancellationToken);
        ...
        loggerMock.Verify(
            x => x.Log(
                LogLevel.Information,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((object v, Type t) => v.ToString().Contains("Handled TestRequest in") && AssertTime(v.ToString(), 100, expectedAccuracyPercent, expectedTimeUnit)),
                null,
                It.IsAny<Func<It.IsAnyType, Exception, string>>()),
            Times.Once);
    ...
    public async Task Handle_ShouldNot_Log_Performance_When_Not_Exceeding_Threshold_WithException()
    {
        ...
        // Simulate a short operation
        var exception = new Exception("fail");
        Task<string> NextWithShortDelay(CancellationToken cancellationToken) => Task.Delay(100, cancellationToken).ContinueWith<string>(_ => throw exception, cancellationToken);
