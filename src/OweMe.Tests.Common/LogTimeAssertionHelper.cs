...
public static class LogTimeAssertionHelper
{
    ...
    {
        var regex = new System.Text.RegularExpressions.Regex($@"\b(\d+)\s*{unit}\b");
        var match = regex.Match(logMessage);
        if (!match.Success)
        {
            throw new System.Exception($"Expected time to match {unit} on {logMessage}");
        }

        if (!long.TryParse(match.Groups[1].Value, out long actualTime))
        {
            throw new System.Exception($"Failed to parse actual time from log message: {logMessage}");
        }
        ...
        {
            throw new System.Exception($"Actual time {actualTime} {unit} is not within the expected range of {lowerBound} to {upperBound} {unit} for log message: {logMessage}");
        }
    }
}